...
//учебный шаблон перевода интерпретации графа из списков смежности (исходящие ребра)
//в матрицу смежности, матрицу инцидентности, список ребер, список смежности (входящие ребра).

program graph_trans_ls_out;
const
   max_p=100;
   max_q=100;
...
begin
   ...
   writeln('translate graph from adjacency list (outcoming) to other');
...
{   
   ...
   begin
      ...
      until (graph_lst_out[i,k]=0)or(k=q);
   end;
   writeln('===========================================');
 }  
   ...
   begin
      k:=1;
      while (graph_lst_out[i,k]<>0)and(k<max_q) do
      begin
         q:=q+1;
         k:=k+1;
      end;
   ...
   begin
      k:=1;
      while (graph_lst_out[i,k]<>0)and(k<=q) do
      begin
         graph_adj[i,graph_lst_out[i,k]]:=1;
         k:=k+1;
      end;
   end;
   ...
   begin
      ...
      begin
         j:=j+1;
         graph_inc[i,j]:=1;
         graph_inc[graph_lst_out[i,k],j]:=-1;
         k:=k+1; 
      end;
   end;
   ...
   begin
      ...
      begin
         j:=j+1;
         graph_edg[j,1]:=i;
         graph_edg[j,2]:=graph_lst_out[i,k];
         k:=k+1;
      end;
   end;
   ...
   //перевод списков смежности (исходящие ребра) в списки смежности (входящие ребра)
   for p_end:=1 to p do
   begin
      j:=1;
      ...
      begin
         k:=1;
         while (graph_lst_out[p_start,k]<>0)and(k<=q) do
         begin
            if graph_lst_out[p_start,k]=p_end then
            begin
               graph_lst_in[p_end,j]:=p_start;
               j:=j+1;
            end;
            k:=k+1;
         end;
